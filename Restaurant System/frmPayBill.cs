        ...
        {
            {
                try
                {
                    String availableString = "A - Available";

                    int selectedOrderRow = ordersDataGridView.CurrentRow.Index;

                    bool orderAvailable = false;
                        
                        if((ordersDataGridView.Rows[selectedOrderRow].Cells[3].Value).ToString() == availableString) 
                        {
                        orderAvailable = true;
                        }
                    ...
                    if (orderAvailable)
                    {
                        (ordersDataGridView.Rows[selectedOrderRow].Cells[3].Value) = "U - Unavailable";
                        MessageBox.Show("Bill Succesfully Paid", "Success!", MessageBoxButtons.OK, MessageBoxIcon.Information);

                    }
                    else if (!orderAvailable)
                    {
                        MessageBox.Show("That Order is unavailable, Please select an available order.", "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }

                }
                catch (Exception ex)
                {
                    MessageBox.Show("Error in Pay Bill Function \n\n ____________________________________ \n\n" + ex.ToString(), "Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    }
